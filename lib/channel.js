...
class Channel extends Struct {
  ...
  asyncOpen (fundingTx, channelSourceIndex = Channel.randomIndex(), channelDestIndex = Channel.randomIndex()) {
    return asink(function * () {
      ...
      let revSecret = yield this.asyncNewSecret()
  ...
  asyncBuildSpending (address, commitment, commitmentTxDepth) {
    return asink(function * () {
      ...
      yield spending.asyncBuild(
        ...
        this.myId,
        commitmentTxDepth,
        this.secretMap) // TODO: filter secretMap to
      return spending
    }, this)
  }
  ...
  asyncPay (amount, htlcSecret, pathIndex = Channel.randomIndex()) {
    return asink(function * () {
      ...
      outputs = yield this.asyncAddOutput(output, outputs)
  ...
  reducePubKeyOutputs (outputs) {
    ...
    for (var key in grouped) {
      ...
      amounts = grouped[key].map((el) => el.amount ? el.amount : 'bottom')
      output = grouped[key][0]
      output.amount = amounts.reduce((acc, cur) => {
   ...
   */
  asyncReduceHtlcOutput (outputs, htlcSecret) {
    return asink(function * () {
      if (yield htlcSecret.asyncSuperCheck()) {
